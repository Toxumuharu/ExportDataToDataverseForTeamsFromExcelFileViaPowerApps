...
## このドキュメントの内容
Excel ファイル内のテーブルのレコードを、既存の Dataverse for Teams へ追加する Power Apps アプリケーションの作成手順 (非公式) です。 Excel ファイルはローカル PC 上にあるものをファイル選択ダイアログ (ファイルピッカー、ファイルを開くダイアログ等呼び方は様々) で指定することができるようにしています。 
